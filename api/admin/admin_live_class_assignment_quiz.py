...
class QuizQuestionOptionInline(nested_admin.NestedTabularInline):
    ...
    class Media:
        css = {
            'all': ('admin/css/quiz_options.css',)
        }
...
class QuizQuestionInline(nested_admin.NestedStackedInline):
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        return (
            ('Question Details', {
                'fields': ('order', 'question_type', 'question_text', 'question_image', 'marks', 'is_active')
            }),
...
class LiveClassAdmin(BaseModelAdmin):
    ...
    class LiveClassAdminForm(forms.ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if self.instance and getattr(self.instance, 'module', None):
                ...
                if getattr(module, 'course', None) and getattr(module.course, 'course', None):
            ...
            self.fields['module'].queryset = CourseModule.objects.filter(is_active=True).select_related('course__course')
        ...
        def clean(self):
            ...
            if course and module:
                ...
                if not (module.course and getattr(module.course, 'course', None) and module.course.course.slug == course.slug):
    ...
    readonly_fields = ['id', 'course_display', 'created_at', 'updated_at', 'get_statistics']
    filter_horizontal = []
    ...
    fieldsets = (
        ...
        ('Schedule', {
            'fields': ('scheduled_date', 'duration_minutes', 'instructor')
        }),
        ('Meeting Details', {
    ...
    class Media:
          js = ('admin/js/vendor/jquery/jquery.min.js', 'admin/js/jquery.init.js', 'admin/js/core.js',
    ...
    def course_name(self, obj):
        """Display course name."""
        return obj.module.course.course.title if obj.module and obj.module.course else '-'
    ...
    def scheduled_date_display(self, obj):
        ...
        if delta.days < 0:
            return format_html(
                ...
                obj.scheduled_date.strftime('%Y-%m-%d %H:%M'),
                abs(delta.days)
            )
        elif delta.days == 0:
            return format_html(
                '<span style="color: #ff9800; font-weight: bold;">{}<br><small>Today!</small></span>',
                obj.scheduled_date.strftime('%H:%M')
            )
        else:
            return format_html(
                '<span>{}<br><small>in {} days</small></span>',
                obj.scheduled_date.strftime('%Y-%m-%d %H:%M'),
                delta.days
            )
    ...
    def get_statistics(self, obj):
        ...
        if obj.pk:
            ...
            avg_duration = obj.attendances.filter(attended=True).aggregate(
            ...
            return format_html(
                ...
                '<p><strong>Attended:</strong> {} ({:.1f}%)</p>'
                '<p><strong>Avg Duration:</strong> {:.0f} minutes</p>'
...
class AssignmentAdmin(BaseModelAdmin):
    ...
    fieldsets = (
        ...
        ('Assignment Information', {
            'fields': ('title', 'description', 'assignment_type', 'order', 'is_active')
        }),
        ('Files & Resources', {
            'fields': ('attachment',),
        ...
        ('Deadline & Submission', {
            'fields': ('due_date', 'late_submission_allowed', 'late_submission_penalty')
        }),
        ('Statistics', {
            'fields': ('get_statistics',),
            'classes': ('collapse',)
    ...
    def due_date_display(self, obj):
        ...
        if delta.days < 0:
            ...
            return format_html(
                ...
                obj.due_date.strftime('%H:%M')
            )
        else:
            return format_html(
                '<span>{}<br><small>{} days left</small></span>',
                obj.due_date.strftime('%Y-%m-%d %H:%M'),
                delta.days
            )
    ...
    def get_statistics(self, obj):
        ...
        if obj.pk:
            ...
            avg_marks = obj.submissions.filter(
                status='graded'
            ...
            return format_html(
                ...
                '<p><strong>Passed:</strong> {} ({:.1f}%)</p>'
                '<p><strong>Average Marks:</strong> {:.1f} / {}</p>'
...
class QuizAdmin(nested_admin.NestedModelAdmin, BaseModelAdmin):
    ...
    fieldsets = (
        ('📚 Course & Module', {
            'fields': ('module',),
            'description': 'Select the module for this quiz'
        }),
        ('Quiz Information', {
            'fields': ('title', 'description', 'is_active')
        }),
        ('Scoring', {
            'fields': ('total_marks', 'passing_marks', 'difficulty')
        }),
        ('Settings', {
            'fields': (
                'duration_minutes',
    ...
    def marks_display(self, obj):
        ...
        return format_html(
            ...
            obj.total_marks,
            obj.passing_marks,
            (obj.passing_marks / obj.total_marks * 100) if obj.total_marks > 0 else 0
        )
    marks_display.short_description = 'Marks'
    ...
    def get_statistics(self, obj):
        ...
        if obj.pk:
            ...
            return format_html(
                ...
                '<p><strong>Passed:</strong> {} ({:.1f}%)</p>'
                '<p><strong>Average Score:</strong> {:.1f} / {} ({:.1f}%)</p>'
...
class QuizQuestionAdmin(BaseModelAdmin):
    ...
    fieldsets = (
        ...
        ('Content', {
            'fields': ('question_text', 'question_image', 'marks')
        }),
        ('Answer', {
            'fields': ('correct_answer_text', 'explanation'),
...
class QuizAnswerAdmin(BaseModelAdmin):
    ...
    fieldsets = (
        ...
        ('Grading', {
            'fields': ('is_correct', 'marks_awarded')
        }),
        ('Metadata', {
            'fields': ('id', 'answered_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
