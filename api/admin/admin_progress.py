...
class ModuleQuizAdmin(BaseAdmin):
    ...
    fieldsets = (
        ("Quiz Information", {"fields": ("module", "title", "description", "order")}),
        ("Settings", {"fields": ("passing_score", "max_attempts", "time_limit_minutes", "is_active")}),
        (
            "Questions",
            {
                "fields": ("questions",),
...
class ModuleAssignmentAdmin(BaseAdmin):
    ...
    readonly_fields = ["id", "created_at", "updated_at", "get_statistics"]
    fieldsets = (
        ("Assignment Information", {"fields": ("module", "title", "description", "attachment_file", "order")}),
        ("Grading", {"fields": ("max_score", "passing_score")}),
        ("Deadline", {"fields": ("due_date", "allow_late_submission")}),
...
class StudentQuizAttemptAdmin(BaseAdmin):
    ...
    fieldsets = (
        ("Attempt Information", {"fields": ("student", "quiz", "attempt_number")}),
        ("Answers & Score", {"fields": ("answers", "score", "is_passed")}),
        ("Timing", {"fields": ("started_at", "completed_at", "get_time_taken_display")}),
        ("Grading", {"fields": ("graded_by", "graded_at", "feedback"), "classes": ("collapse",)}),
        ("Metadata", {"fields": ("id", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class StudentAssignmentSubmissionAdmin(BaseAdmin):
    ...
    fieldsets = (
        ("Submission Information", {"fields": ("student", "assignment", "submitted_at", "is_late", "resubmission_count")}),
        ("Submission Content", {"fields": ("submission_file", "submission_text", "submission_url")}),
        ("Grading", {"fields": ("status", "score", "feedback", "graded_by", "graded_at")}),
        ("Metadata", {"fields": ("id", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def status_display(self, obj):
        ...
        return format_html(
            '<span style="color: {}; font-weight: bold;">{}</span>', colors.get(obj.status, "black"), obj.get_status_display()
        )
...
class StudentModuleProgressAdmin(BaseAdmin):
    ...
    fieldsets = (
        ("Progress Information", {"fields": ("student", "module", "is_completed", "completed_at")}),
        ("Teacher Notes", {"fields": ("marked_by", "notes")}),
        ("Metadata", {"fields": ("id", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class CourseProgressAdmin(BaseAdmin):
    ...
    fieldsets = (
        ("Progress Information", {"fields": ("student", "course", "enrollment")}),
        (
            "Completion Details",
            {
                "fields": (
        ...
        ("Certificate", {"fields": ("certificate_issued", "certificate_issued_at"), "classes": ("collapse",)}),
        ("Metadata", {"fields": ("id", "created_at", "updated_at", "last_activity_at"), "classes": ("collapse",)}),
    )
    ...
    def issue_certificates(self, request, queryset):
        ...
        updated = queryset.filter(is_completed=True, certificate_issued=False).update(
            certificate_issued=True, certificate_issued_at=timezone.now()
        )
