...
class Order(TimeStampedModel):
    ...
    order_number = models.CharField(
        max_length=100, unique=True, editable=False, db_index=True, help_text="Unique order number (auto-generated)"
    )
    user = models.ForeignKey(
        "api.CustomUser", on_delete=models.CASCADE, related_name="orders", help_text="User who placed this order"
    )
    ...
    currency = models.CharField(max_length=3, default="BDT", help_text="Currency code (BDT, USD, etc.)")
    ...
    coupon_code = models.CharField(max_length=50, blank=True, help_text="Snapshot of coupon code at time of order")

    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="pending", db_index=True, help_text="Current status of the order"
    )
    ...
    # Billing info
    billing_email = models.EmailField(help_text="Email for billing and receipts")
    billing_name = models.CharField(max_length=200, help_text="Full name for billing")
    billing_phone = models.CharField(max_length=20, blank=True, help_text="Contact phone number")
    billing_address = models.TextField(blank=True, help_text="Billing address (optional)")
    billing_city = models.CharField(max_length=100, default="Dhaka", help_text="Billing city")
    billing_country = models.CharField(max_length=100, default="Bangladesh", help_text="Billing country")
    billing_postcode = models.CharField(max_length=20, default="1207", help_text="Billing postal/ZIP code")
    ...
    # Active status
    is_active = models.BooleanField(default=True, db_index=True, help_text="Whether this order is active")
...
class PaymentTransaction(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    installment = models.ForeignKey("OrderInstallment", on_delete=models.CASCADE, related_name="payment_transactions")
...
class OrderInstallment(TimeStampedModel):
    ...
    # Active status
    is_active = models.BooleanField(default=True, db_index=True, help_text="Whether this installment is active")

    extra_data = models.JSONField(default=dict, blank=True, help_text="Any Extra info metadata (session keys, etc.)")
    ...
    def _update_next_installment_date(self):
        ...
        Order.objects.filter(id=self.order.id).update(next_installment_date=next_due)
...
class OrderItem(TimeStampedModel):
    ...
    currency = models.CharField(max_length=3, default="BDT", help_text="Currency for this item")

    # Active status
    is_active = models.BooleanField(default=True, db_index=True, help_text="Whether this order item is active")
