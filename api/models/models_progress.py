...
class StudentModuleProgress(TimeStampedModel):
    ...
    module = models.ForeignKey(
        "api.CourseModule", related_name="student_progress", on_delete=models.CASCADE, help_text="Module being tracked"
    )
    is_completed = models.BooleanField(default=False, help_text="Whether student completed this module")
    completed_at = models.DateTimeField(null=True, blank=True, help_text="When module was marked complete")
    ...
    def check_auto_complete(self):
        ...
        for assignment in active_assignments:
            if not assignment.submissions.filter(student=self.student, status="approved").exists():
                return False
...
class CourseProgress(TimeStampedModel):
    ...
    course = models.ForeignKey(
        "api.Course", related_name="student_progress", on_delete=models.CASCADE, help_text="Course being tracked"
    )
    ...
    certificate_issued_at = models.DateTimeField(null=True, blank=True, help_text="When certificate was issued")
    ...
    def calculate_completion(self):
        ...
        self.modules_completed = StudentModuleProgress.objects.filter(
            student=self.student, module__in=modules, is_completed=True
        ).count()
    ...
    def get_next_incomplete_module(self):
        ...
        for module in modules:
            progress, _ = StudentModuleProgress.objects.get_or_create(student=self.student, module=module)
