...
class CustomPayment(TimeStampedModel):
    ...
    student = models.ForeignKey(
        "api.CustomUser", on_delete=models.CASCADE, related_name="custom_payments", help_text="Student being enrolled"
    )
    ...
    # Payment Details
    title = models.CharField(max_length=255, help_text="Payment title (e.g., 'Scholarship Enrollment', 'Staff Training')")
    ...
    original_price = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Original course price (for reference)"
    )
    currency = models.CharField(max_length=3, default="BDT", help_text="Currency code")

    # Status and Payment
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="pending", db_index=True, help_text="Payment status"
    )
    ...
    cancelled_at = models.DateTimeField(null=True, blank=True, help_text="When payment was cancelled")

    # Admin notes
    notes = models.TextField(blank=True, help_text="Internal notes (staff only)")

    # Active status
    is_active = models.BooleanField(default=True, db_index=True, help_text="Whether this payment record is active")
    ...
    def create_enrollment(self):
        ...
        if self.course and not self.enrollment:
            enrollment = Enrollment.objects.create(user=self.student, course=self.course, is_active=True)
