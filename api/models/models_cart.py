...
class Cart(TimeStampedModel):
    ...
    session_key = models.CharField(
        max_length=40, null=True, blank=True, db_index=True, help_text="Session key for guest users"
    )
...
class CartItem(TimeStampedModel):
    ...
    course = models.ForeignKey(Course, on_delete=models.CASCADE, help_text="Course added to cart")
    batch = models.ForeignKey(
        "CourseBatch", on_delete=models.CASCADE, null=True, blank=True, help_text="Specific batch for enrollment (optional)"
    )
...
class Wishlist(TimeStampedModel):
    ...
    courses = models.ManyToManyField(Course, related_name="wishlisted_by", blank=True, help_text="Courses saved for later")
