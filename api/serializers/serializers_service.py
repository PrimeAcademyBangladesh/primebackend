...
class ContentSectionListSerializer(serializers.ModelSerializer):
    ...
    page = serializers.SlugRelatedField(
        slug_field="name", queryset=PageService.objects.all(), help_text="Select a page by name"
    )

    # Human readable displays
    position_display = serializers.CharField(source="get_position_choice_display", read_only=True)
...
class ContentSectionCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_button_link(self, value):
        ...
        if self.initial_data.get("button_text") and not value:
            raise serializers.ValidationError("Button link is required when button text is provided.")
        return value
    ...
    def validate(self, data):
        ...
        if section_type == "icon" and media_type == "video":
            raise serializers.ValidationError({"media_type": "Icon sections can only use images."})

        # Validate content length for icon sections
        if section_type == "icon" and content and len(content) > 500:
            raise serializers.ValidationError({"content": "Icon sections should have concise content (max 500 characters)."})
        ...
        if media_type == "video" and section_type == "info":
            ...
            if not data.get("video_provider"):
                raise serializers.ValidationError({"video_provider": "Video provider is required when media type is video."})
            # Only require thumbnail for new instances
            if not data.get("video_thumbnail") and not self.instance:
                raise serializers.ValidationError({"video_thumbnail": "Video thumbnail is required when media type is video."})
