...
from api.utils.export_utils import CSVExporter, InvoicePDFGenerator, PDFExporter
...
def validate_and_parse_date_range(request, max_range_days=365):
    ...
    if start_date > end_date:
        return None, None, Response({"error": "start_date cannot be after end_date"}, status=status.HTTP_400_BAD_REQUEST)
...
def export_students_pdf(request):
    ...
    content = []
    content.append(Paragraph(f"Total Students: {queryset.count()}", exporter.styles["CustomHeading"]))
    content.append(
        Paragraph(
            f"Active: {queryset.filter(is_enabled=True).count()} | " f"Disabled: {queryset.filter(is_enabled=False).count()}",
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            name="order_id", type=OpenApiTypes.UUID, location=OpenApiParameter.PATH, description="Order UUID", required=True
        ),
    ],
    responses={
...
def export_my_order_invoice_by_number_pdf(request, order_number):
    ...
    try:
        ...
        if request.user.role == "student":
            ...
            # Staff/Admin can access any order
            order = (
                Order.objects.select_related("user", "coupon").prefetch_related("items__course").get(order_number=order_number)
            )
    except Order.DoesNotExist:
        return Response({"error": "Order not found"}, status=status.HTTP_404_NOT_FOUND)
...
def export_revenue_analytics_pdf(request):
    ...
    content.append(Paragraph(f"<b>Total Discounts Given:</b> ৳{total_discount:,.2f}", exporter.styles["CustomBody"]))
    content.append(Spacer(1, 20))
    ...
    if top_courses:
        ...
        for course in top_courses:
            if course["course_title"]:  # Skip null courses
                course_data.append([course["course_title"][:50], str(course["sales_count"]), f"৳{course['revenue']:,.2f}"])
