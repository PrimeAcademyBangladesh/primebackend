...
from api.serializers.serializers_order import (EnrollmentCreateSerializer,
...
class EnrollmentViewSet(BaseAdminViewSet):
    ...
    def get_permissions(self):
        ...
        """
        if self.action in ['list', 'retrieve', 'my_enrollments', 'access_course', 'complete']:
    ...
    def statistics(self, request):
        ...
        stats = {
            ...
            'average_progress': queryset.aggregate(
                avg=Avg('progress_percentage')
            )['avg'] or 0,
            'enrollments_by_course': queryset.values(
                'course__title'
            ).annotate(
                count=Count('id')
            ).order_by('-count')[:10]
