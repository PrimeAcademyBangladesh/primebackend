...
from api.utils.cache_utils import CACHE_KEY_FAQ_LIST, cache_response
...
class FAQViewSet(BaseAdminViewSet):
    ...
    def list(self, request):
        items = FAQItem.objects.all().prefetch_related("faqs").order_by("order", "created_at")

        serializer = FAQItemSerializer(items, many=True)
        return Response(
            {"success": True, "message": "FAQs retrieved successfully", "data": serializer.data}, status=status.HTTP_200_OK
        )
