...
class PDFExporter:
    ...
    def create_pdf(self, content_elements):
        ...
        doc = SimpleDocTemplate(
            buffer, pagesize=self.pagesize, rightMargin=72, leftMargin=72, topMargin=72, bottomMargin=18, title=self.title
        )
...
class InvoicePDFGenerator:
    ...
    def _register_styles(self):
        ...
        self.styles.add(ParagraphStyle(name="InvBodyText", fontSize=10, leading=14, textColor=colors.HexColor("#4a5568")))
    ...
    def generate_qr_code(self):
        ...
        try:
            ...
            drawing = Drawing(60, 60, transform=[60.0 / width, 0, 0, 60.0 / height, 0, 0])
            drawing.add(qr_code)
            return drawing
        except Exception:
            return None
    ...
    def generate(self):
        ...
        doc = SimpleDocTemplate(
            self.buffer, pagesize=self.pagesize, rightMargin=50, leftMargin=50, topMargin=50, bottomMargin=50
        )
        ...
        header_data = [[logo, Paragraph(f"<b>Invoice #:</b> {self.order.order_number}", self.styles["InvoiceRight"])]]
        ...
        story.append(
            Paragraph(
                "<b>INVOICE</b>",
                ParagraphStyle(name="InvoiceTitle", fontSize=24, fontName="Helvetica-Bold", alignment=TA_CENTER),
            )
        )
        ...
        from_to_data = [
            ...
            [
                Paragraph(
                    "<b>Prime Academy</b><br/>Suite 5040, Lift 5<br/>Dhanmondi, Dhaka<br/>Phone: +880 1300 290492<br/>Email: info@primeacademy.org",
                    self.styles["InvBodyText"],
                ),
                Paragraph(
                    f"<b>{self.order.billing_name}</b><br/>{self.order.billing_email}<br/>{self.order.billing_phone}"
        ...
        info_text = (
            f'<b>Date:</b> {self.order.created_at.strftime("%d %B %Y")}<br/>'
        ...
        story.append(
            Paragraph(
                '<font color="#003366"><b>Payment Details</b></font>',
                ParagraphStyle(
                    name="PaymentHeading", fontSize=14, fontName="Helvetica-Bold", textColor=colors.HexColor("#003366")
                ),
            )
        )
        story.append(Spacer(1, 10))
        ...
        for item in self.order.items.all():
            ...
            try:
                ...
                enrollment = Enrollment.objects.filter(user=self.order.user, course=item.course).first()
            ...
            payment_data.append([item.course.title, batch_name, f"{item.price:,.2f}/-"])
        ...
        if qr_code:
            qr_data = [
                [
                    qr_code,
                    Paragraph(
                        "<b>Verify this invoice</b><br/>Scan QR code to verify authenticity<br/>"
        ...
        story.append(Spacer(1, 10))
        footer_text = (
            "<b>Thank you for choosing Prime Academy!</b><br/><br/>"
        ...
        story.append(
            Paragraph(
                automated_notice,
                ParagraphStyle(name="AutomatedNotice", fontSize=9, textColor=colors.HexColor("#666666"), alignment=TA_CENTER),
            )
        )
