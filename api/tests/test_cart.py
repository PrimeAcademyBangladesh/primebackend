...
from api.models.models_course import Course, Category
...
class CartAPITestCase(APITestCase):
    ...
    def test_add_duplicate_to_cart(self):
        ...
        # Add first time
        response1 = self.client.post('/api/cart/add/', {
            'course_id': str(self.course.id)
        }, format='json')
    ...
    def test_add_inactive_course_to_cart(self):
        ...
        response = self.client.post('/api/cart/add/', {
            'course_id': str(inactive_course.id)
        }, format='json')
...
class WishlistAPITestCase(APITestCase):
    ...
    def test_add_duplicate_to_wishlist(self):
        ...
        # Add first time
        response1 = self.client.post('/api/wishlist/add/', {
            'course_id': str(self.course.id)
        }, format='json')
        self.assertEqual(response1.status_code, status.HTTP_200_OK)
        
        # Add second time
        response2 = self.client.post('/api/wishlist/add/', {
            'course_id': str(self.course.id)
        }, format='json')
        self.assertEqual(response2.status_code, status.HTTP_200_OK)
