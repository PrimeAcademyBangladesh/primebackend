...
class TeacherLoginTests(TestCase):
    ...
    def test_inactive_teacher_cannot_login(self):
        ...
        self.assertTrue(
            'disabled' in detail_text.lower() or
            'unable to log in' in detail_text.lower() or
            'invalid credentials' in detail_text.lower(),
...
class TeacherProfileAndPasswordTests(TestCase):
    def setUp(self):
        ...
        resp = self.client.post(login_url, {'email': 'profile_teacher@example.com', 'password': 'initialPass123'}, format='json')
    ...
    def test_change_password_success(self):
        ...
        login_url = reverse('teacher-login')
        bad = self.client.post(login_url, {'email': 'profile_teacher@example.com', 'password': 'initialPass123'}, format='json')
...
class StudentAuthTests(TestCase):
    ...
    def test_student_login_and_profile_and_logout(self):
        ...
        student = CustomUser.objects.create_user(
            email='student2@example.com', password='Password123!', first_name='Two', last_name='Student', phone='01700000011', role=CustomUser.Role.STUDENT, is_active=True
        )
        ...
        patch = self.client.patch(self.profile_url, {'first_name': 'UpdatedStudent', 'email': 'attacker@example.com', 'phone': '00000000000'}, format='json')
    ...
    def test_password_reset_request_and_confirm(self):
        student = CustomUser.objects.create_user(
            email='student3@example.com', password='OrigPass1', first_name='Three', last_name='Student', phone='01700000012', role=CustomUser.Role.STUDENT, is_active=True
        )
        ...
        resp2 = self.client.post(self.password_reset_confirm_url + f'?token={token}', {'new_password': 'NewPass123!', 'new_password2': 'NewPass123!'}, format='json')
        self.assertEqual(resp2.status_code, 200)
    ...
    def test_change_password_requires_auth_and_valid_old(self):
        student = CustomUser.objects.create_user(
            email='student4@example.com', password='OldPass1', first_name='Four', last_name='Student', phone='01700000013', role=CustomUser.Role.STUDENT, is_active=True
        )
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {access}')
        # Wrong old password
        resp = self.client.post(self.change_password_url, {'old_password': 'wrong', 'new_password': 'X1Y2Z3!8', 'new_password2': 'X1Y2Z3!8'}, format='json')
        self.assertEqual(resp.status_code, 400)

        # Correct old password
        resp2 = self.client.post(self.change_password_url, {'old_password': 'OldPass1', 'new_password': 'X1Y2Z3!8', 'new_password2': 'X1Y2Z3!8'}, format='json')
        self.assertEqual(resp2.status_code, 200)
