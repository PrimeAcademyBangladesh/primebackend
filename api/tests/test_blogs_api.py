...
class BlogAndCategoryIntegrationTests(TestCase):
    ...
    def test_blog_create_and_retrieve_flow(self):
        ...
        blog_obj = Blog.objects.create(
            category=cat_obj, title="Integration Blog", excerpt="Integrate", content="Integration content", status="published"
        )
        slug = blog_obj.slug
    ...
    def test_blog_update_flow(self):
        ...
        blog_obj = Blog.objects.create(
            category=cat1_obj, title="Blog To Update", excerpt="Before", content="Before content", status="published"
        )
    ...
    def test_draft_blogs_hidden_from_non_staff(self):
        ...
        draft_blog = Blog.objects.create(
            category=self.category, title="Draft Blog", excerpt="Draft excerpt", content="Draft content", status="draft"
        )
    ...
    def test_latest_blogs_endpoint(self):
        ...
        blog1 = Blog.objects.create(
            category=self.category, title="Oldest Blog", excerpt="Oldest excerpt", content="Oldest content", status="published"
        )
        blog1.published_at = now - timedelta(days=3)
        blog1.save()

        blog2 = Blog.objects.create(
            category=self.category, title="Middle Blog", excerpt="Middle excerpt", content="Middle content", status="published"
        )
        blog2.published_at = now - timedelta(days=1)
        blog2.save()

        blog3 = Blog.objects.create(
            category=self.category, title="Newest Blog", excerpt="Newest excerpt", content="Newest content", status="published"
        )
        ...
        Blog.objects.create(
            category=self.category, title="Draft Blog Latest", excerpt="Draft excerpt", content="Draft content", status="draft"
        )
