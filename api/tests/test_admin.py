...
class AdminViewSetTests(TestCase):
    def setUp(self):
        ...
        self.teacher = CustomUser.objects.create_user(
            email="teacher_admin_test@example.com",
            password="TeachPass1",
            first_name="T",
            last_name="User",
            phone="01700000021",
        ...
        for i in range(3):
            CustomUser.objects.create_user(
                email=f"student_admin_{i}@example.com",
                password="S1234567",
                first_name="S",
                last_name=str(i),
                phone=f"0170000002{i+2}",
        ...
        resp = self.client.post(
            reverse("admin-login"), {"email": "admin@example.com", "password": "AdminPass1"}, format="json"
        )
    ...
    def test_admin_teacher_crud(self):
        ...
        data = {
            "email": "managed_teacher@example.com",
            "password": "TeachNew1",
            "first_name": "Managed",
            "last_name": "Teacher",
            "phone": "01700000999",
...
class AdminPermissionTests(TestCase):
    def setUp(self):
        ...
        self.teacher = CustomUser.objects.create_user(
            email="plain_teacher@example.com",
            password="TeachPass2",
            first_name="T2",
            last_name="User",
            phone="01700000022",
    ...
    def test_non_admin_cannot_access_admin_endpoints(self):
        # login as teacher
        resp = self.client.post(
            reverse("teacher-login"), {"email": "plain_teacher@example.com", "password": "TeachPass2"}, format="json"
        )
        self.assertEqual(resp.status_code, 200)
        tokens = resp.data.get("tokens") or resp.data.get("data", {}).get("tokens")
    ...
    def test_admin_can_list_teachers_includes_profile(self):
        ...
        for i in range(2):
            CustomUser.objects.create_user(
                email=f"teach_list_{i}@example.com",
                password="TeachList1",
                first_name="TL",
                last_name=str(i),
                phone=f"0170000023{i}",
                role=CustomUser.Role.TEACHER,
                is_active=True,
            )
        resp = self.client.post(
            reverse("admin-login"), {"email": "list_admin@example.com", "password": "AdminList1"}, format="json"
        )
        self.assertEqual(resp.status_code, 200)
        tokens = resp.data.get("tokens") or resp.data.get("data", {}).get("tokens")
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {tokens['access']}")
