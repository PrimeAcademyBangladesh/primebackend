...
class PhoneChangeTests(TestCase):
    def setUp(self):
        ...
        resp = self.client.post(
            reverse("student-login"), {"email": self.student.email, "password": "testpass123"}, format="json"
        )
        access = None
        if resp.data:
    ...
    def test_non_student_cannot_update(self):
        # login as teacher
        resp = self.client.post(
            reverse("teacher-login"), {"email": self.teacher.email, "password": "teacherpass123"}, format="json"
        )
        access = None
        if resp.data:
            data_block = resp.data.get("data") or {}
